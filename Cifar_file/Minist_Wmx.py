...
def body2(i, x, out):
    ...
    #     maxpooling
    pool1 = func.weight_pool(p=p1, f=f1, reduce_fun=tf.reduce_max, pool_fun=func.max_pool_with_mask)
...
np.save('../output/cwmx_cost',np.array(cost_save))
np.save('../output/cwmx_acc', np.array(accuracy_save))
