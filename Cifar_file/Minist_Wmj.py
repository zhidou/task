...
def body2(i, x, out):
    ...
    #     maxpooling
    pool1 = func.weight_pool(p=p1, f=f1, reduce_fun=tf.reduce_max, pool_fun=func.majority_pool_with_mask)
    ...
    #     maxpooling
    pool2 = func.weight_pool(p=p2, f=f2, reduce_fun=tf.reduce_max, pool_fun=func.majority_pool_with_mask)
...
np.save('../output/cwmj_cost',np.array(cost_save))
np.save('../output/cwmj_acc', np.array(accuracy_save))
