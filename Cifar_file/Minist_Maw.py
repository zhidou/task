...
def body2(i, x, out):
    ...
    #     maxpooling
    pool1 = func.pool_weight(p=p1, f=f1, reduce_fun=tf.reduce_max, pool_fun=func.max_pool_with_mask)
    ...
    #     maxpooling
    pool2 = func.pool_weight(p=p2, f=f2, reduce_fun=tf.reduce_max, pool_fun=func.max_pool_with_mask)
...
np.save('../output/cmaw_cost',np.array(cost_save))
np.save('../output/cmaw_acc', np.array(accuracy_save))
