...
# get batch just for cifar
def get_batch(batch_size, data_x, data_y, index):
    i = 0
    x = []
    y = []
    while i < batch_size:
...
def weight_pool_original_with_mask(p, f, reduce_fun, pool_fun):
    ...
    [N,H,W,K,C] = p.get_shape().as_list()
    pool = tf.reduce_max(tf.multiply(p, mask), axis=3)
    mask = tf.cast(tf.equal(p, tf.reshape(pool, [N,H,W,1,C])), dtype=tf.float32)
    mask = tf.div(mask, tf.reduce_sum(mask, axis=3, keep_dims=True))
    return pool, mask

def weight_pool_original(p, f, reduce_fun, pool_fun):
    pool, mask = weight_pool_original_with_mask(p, f, reduce_fun, pool_fun)
    return pool
