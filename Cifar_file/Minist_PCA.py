...
inputxs = tf.TensorArray(dtype=tf.float32, size=train_iter, clear_after_read=True).split(x, [mini_batch] * train_iter)
...
np.save('../output/cpca_cost',np.array(cost_save))
np.save('../output/cpca_acc', np.array(accuracy_save))
